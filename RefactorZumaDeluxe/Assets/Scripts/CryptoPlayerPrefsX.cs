...
public class CryptoPlayerPrefsX
{
	...
	public static bool SetBool(string name, bool value)
	{
		try
		{
			SecurePlayerPrefs.SetInt(name, value ? 1 : 0);
		}
		catch
		{
			return false;
	...
	{
		return SecurePlayerPrefs.GetInt(name) == 1;
	}

	public static bool GetBool(string name, bool defaultValue)
	{
		if (SecurePlayerPrefs.HasKey(name))
		{
			return GetBool(name);
		}
		return defaultValue;
	...
	public static bool[] GetBoolArray(string key)
	{
		if (SecurePlayerPrefs.HasKey(key))
		{
			byte[] array = Convert.FromBase64String(SecurePlayerPrefs.GetString(key));
			if (array.Length < 6)
			{
	...
	public static bool SetStringArray(string key, string[] stringArray)
	{
		...
		try
		{
			SecurePlayerPrefs.SetString(key, Convert.ToBase64String(array) + "|" + string.Join(string.Empty, stringArray));
		}
		catch
		{
			return false;
	...
	private static bool SaveBytes(string key, byte[] bytes)
	{
		try
		{
			SecurePlayerPrefs.SetString(key, Convert.ToBase64String(bytes));
		}
		catch
		{
			return false;
