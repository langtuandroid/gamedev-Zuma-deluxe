...
public class Sphere : MonoBehaviour
{
		...
		if (gameObject.tag == "Sphere")
		{
			...
			if (status == Status.Flying && component.status == Status.Moving)
			{
				...
				if (type == Type.Bomb)
				{
					...
					SoundController.instance.Play(SoundController.Others.Explosion);
					return;
				}
				if (type == Type.Color)
				{
					...
					SoundController.instance.Play(SoundController.Others.ChangeColor);
					return;
				}
				SoundController.instance.Play(SoundController.Others.Hit);
				GameState.EndFlying();
				BeginJoining();
				int num = 0;
	...
	public void MyDestroy()
	{
		...
		else
		{
			...
			{
				MainGameController.instance.IncreaseFallingHole();
			}
		}
		if (onDestroyed != null)
		{
