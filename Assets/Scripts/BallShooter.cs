...
public class BallShooter : MonoBehaviour
{
	...
	[FormerlySerializedAs("uiEvent")] public InterfaceAction interfaceAction;
	...
	private void Start()
	{
		...
		{
			Timekeeper.Schedule(this, 2f, delegate
			{
	...
	public void SetUp(BattleActions battleActions)
	{
		type = battleActions.shooterType;
		if (type == Type.Rotate)
		{
			base.transform.parent.position = battleActions.cannonPosition.position;
		}
	}
	...
	private void ExchangeBalls()
	{
		if (!(slots[0].sphere == null) && !(slots[1].sphere == null))
		{
            ...
            Timekeeper.Schedule(this, 0.2f, delegate
			{
				isExchanging = false;
				if (applyUpdate != null)
				{
	...
	private void Shoot(Vector3 touchPoint)
	{
		if (!(slots[0].sphere == null) && GameState.IsNormal())
		{
			...
			slots[0].sphere = null;
			ReplaceBall();
			MainManager.instance.OnCannonShoot();
	...
	private void Update()
	{
		if (Input.GetMouseButton(0) && !isCannonPointerDown && !CUtils.IsPointerOverUIObject() && !ConversationManager.instance.IsDialogShowing() && MainManager.instance.status == MainManager.Status.Playing && MonoUtils.instance.numDroppingBomb == 0)
		{
