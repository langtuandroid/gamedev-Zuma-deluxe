...
public class Cover : MonoBehaviour
{
	...
	private void Start()
	{
		...
		instance.onScreenSizeChanged = (Action)Delegate.Combine(instance.onScreenSizeChanged, new Action(RefreshCover));
	}
	...
	{
		float width = InterfaceCamera.instance.GetWidth();
		float height = InterfaceCamera.instance.GetHeight();
		float virtualWidth = InterfaceCamera.instance.virtualWidth;
		float virtualHeight = InterfaceCamera.instance.virtualHeight;
		...
		float num3 = (!InterfaceCamera.instance.landscape) ? 800 : 400;
		float y = (!InterfaceCamera.instance.landscape) ? 400 : 800;
