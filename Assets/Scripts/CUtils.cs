...
public class CUtils
{
	...
	public static BaseManager GetGameController()
	{
		GameObject gameObject = GameObject.FindWithTag("GameController");
		return gameObject.GetComponent<BaseManager>();
	}
	...
	{
		float num = InterfaceCamera.instance.GetWidth() + padding;
		float num2 = InterfaceCamera.instance.GetHeight() + padding;
	...
	public static void LoadScene(int sceneIndex, bool useScreenFader = false)
	{
		if (useScreenFader)
		{
			FadeScreen.instance.GotoScene(sceneIndex);
		}
		else
		{
