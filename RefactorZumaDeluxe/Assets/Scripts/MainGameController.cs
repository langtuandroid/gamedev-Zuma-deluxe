...
public class MainGameController : BaseGameController
{
	...
	public void SpawnRandomItem(List<Sphere> explodedBalls)
	{
		if (explodedBalls.Count >= attacks.itemConfig.minExplodeBalls && !((float)Random.Range(0, 100) > attacks.itemConfig.randomCoef) && numItems < attacks.itemConfig.maxItems && !(lastItemTime + attacks.itemConfig.gapTime > Time.time))
		{
			...
			if (Mathf.Abs(position.x) < UICameraControllerController.instance.GetWidth() - 0.3f && Mathf.Abs(position.y) < UICameraControllerController.instance.GetHeight() - 1f)
			{
	...
	public void OnComplete()
	{
		...
		CompleteDialogBox completeDialogBox = (CompleteDialogBox)UIManager.instance.GetDialog(DialogType.Complete);
		completeDialogBox.numStars = progress.GetReachedTarget();
		completeDialogBox.score = (int)progress.Current;
		completeDialogBox.isWin = isWin;
		UIManager.instance.ShowDialog(completeDialogBox);
		if (isWin)
		{
			...
			if (level == LevelController.GetUnlockLevel())
			{
				...
				BalanceController.CreditBalance(3);
			}
			SoundController.instance.Play(SoundController.Others.Win);
			if (level % 2 == 0)
			{
		...
		{
			SoundController.instance.Play(SoundController.Others.Fail);
			CUtils.ShowInterstitialAd();
		}
	}
	...
	public void GiveCompliment(Vector3 beginPosition, int bonusScore)
	{
		...
		gameObject.GetComponent<ExtraText>().message = "+" + bonusScore;
