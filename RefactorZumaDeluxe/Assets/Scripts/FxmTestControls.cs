...
public class FxmTestControls : MonoBehaviour
{
	...
	protected float[] m_fPlayToolbarTimes = new float[8]
	{
		1f,
		1f,
		1f,
	...
	private void Update()
	{
		...
		{
			DelayCreateInstanceEffect(bEndMove: false);
			return;
		}
		NgObject.GetMeshInfo(NcEffectBehaviour.GetRootInstanceEffect(), bInChildren: true, out m_nVertices, out m_nTriangles, out m_nMeshCount);
		m_nParticleCount = 0;
	...
	private bool IsAliveAnimation()
	{
		...
		Transform[] componentsInChildren = rootInstanceEffect.GetComponentsInChildren<Transform>(includeInactive: true);
	...
	private void winActionToolbar(int id)
	{
		...
		if (FXMakerLayout.m_bMinimizeAll || m_bMinimize)
		{
			...
			{
				message = $"P={m_nParticleCount} M={m_nMeshCount} T={m_nTriangles}";
				str = $"ParticleCount = {m_nParticleCount} MeshCount = {m_nMeshCount}\n Mesh: Triangles = {m_nTriangles} Vertices = {m_nVertices}";
			}
			GUI.Box(FXMakerLayout.GetInnerHorizontalRect(childVerticalRect, num, 0, 2), message);
			if (FxmTestMain.inst.IsCurrentEffectObject())
			{
		...
		{
			message = $"P = {m_nParticleCount}\nM = {m_nMeshCount}\nT = {m_nTriangles}";
			str = $"ParticleCount = {m_nParticleCount} MeshCount = {m_nMeshCount}\n Mesh: Triangles = {m_nTriangles} Vertices = {m_nVertices}";
		}
		GUI.Box(FXMakerLayout.GetInnerHorizontalRect(childVerticalRect, num, 0, 1), new GUIContent(message, NgTooltipTooltip(str)));
		if (FxmTestMain.inst.IsCurrentEffectObject())
		{
			bool flag = false;
			...
			if (flag)
			{
				FxmTestMain.inst.CreateCurrentInstanceEffect(bRunAction: false);
		...
		gUIContent2.message = gUIContent2.message + " " + m_fDistPerTime.ToString("00.00");
		...
		if (NgLayout.GUIButton(FXMakerLayout.GetInnerHorizontalRect(childVerticalRect, num, 9, 1), new GUIContent("Multi", m_nMultiShotCount.ToString()), bEnable: true))
		{
		...
		gUIContent3.message = gUIContent3.message + " " + m_fTimeScale.ToString("0.00");
		GUI.Box(FXMakerLayout.GetInnerHorizontalRect(childVerticalRect, num, 0, 2), gUIContent);
		innerHorizontalRect = FXMakerLayout.GetInnerHorizontalRect(childVerticalRect, num, 2, 5);
		innerHorizontalRect.y += 5f;
		...
		if (FxmTestMain.inst.IsCurrentEffectObject())
		{
			...
			gUIContent4.message = gUIContent4.message + " " + (Time.time - m_fPlayStartTime).ToString("0.000");
			GUI.Box(FXMakerLayout.GetInnerHorizontalRect(childVerticalRect, num, 0, 2), gUIContent);
			innerHorizontalRect = FXMakerLayout.GetInnerHorizontalRect(childVerticalRect, num, 2, 5);
			innerHorizontalRect.y += 5f;
	...
	{
		DelayCreateInstanceEffect(bEndMove: true);
	}
	...
	private void NextInstanceEffect()
	{
		...
		{
			FxmTestMain.inst.ChangeEffect(bNext: true);
		}
		else
		{
			CreateInstanceEffect();
	...
	private void CreateInstanceEffect()
	{
		if (FxmTestMain.inst.IsCurrentEffectObject())
		{
			FxmTestMain.inst.CreateCurrentInstanceEffect(bRunAction: true);
		}
	}
