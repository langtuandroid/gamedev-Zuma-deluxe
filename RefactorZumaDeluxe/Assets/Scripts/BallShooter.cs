...
public class BallShooter : MonoBehaviour
{
	...
	[FormerlySerializedAs("uiEvent")] public UIEventListener uiEventListener;
	...
	private void Start()
	{
		...
		uIEventListener.onMouseDown = (Action)Delegate.Combine(uIEventListener.onMouseDown, new Action(OnCannonPointerDown));
		UIEventListener uIEvent2 = uiEventListener;
	...
	public void CreateMultiColors()
	{
		...
		if (!isReplacing && !isExchanging)
		{
			...
			{
				SpawnBalls(Sphere.Type.MultiColor, 1);
				applyUpdate = null;
				GameState.EndFlying();
			});
		}
	...
	private void Shoot(Vector3 touchPoint)
	{
		if (!(slots[0].sphere == null) && GameState.IsNormal())
		{
			...
			slots[0].sphere = null;
			ReplaceBall();
			MainGameController.instance.OnCannonShoot();
			SoundController.instance.Play(SoundController.Others.Shoot);
		}
	}

	private void Update()
	{
		if (Input.GetMouseButton(0) && !isCannonPointerDown && !CUtils.IsPointerOverUIObject() && !UIManager.instance.IsDialogShowing() && MainGameController.instance.status == MainGameController.Status.Playing && MonoUtils.instance.numDroppingBomb == 0)
		{
